...
import { Users, Stethoscope, Activity, TrendingUp, Plus, Search, Edit, Trash2, CheckCircle2, X, BarChart3, Brain, Sparkles } from "lucide-react";
...
export default function AdminPanel({ user }) {
  ...
  useEffect(() => {
    ...
    if (hash === "doctors" || hash === "users" || hash === "overview" || hash === "analytics") {
  ...
  const handleDeleteDoctor = async (doctorUid: string, doctorName: string) => {
    const confirmed = window.confirm(`هل أنت متأكد من حذف حساب ${doctorName}؟ هذا الإجراء لا يمكن التراجع عنه.`);
    if (!confirmed) return;

    try {
      const response = await fetch(`http://127.0.0.1:8000/api/admin/delete-doctor/${doctorUid}`, {
        method: "DELETE",
      });

      const data = await response.json();
      if (!response.ok) throw new Error(data.detail || "Failed to delete doctor");

      // مفيش داعي لـ setDoctors يدويًا هنا، لأن onSnapshot هيحدث القايمة تلقائيًا
    } catch (error: any) {
      console.error("Error deleting doctor:", error);
      alert(error.message || "حصل خطأ أثناء حذف الحساب");
    }
  };
    ...
    return (
      <div className="relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-card/90 border border-border shadow-elevated backdrop-blur-md">
            <Sparkles className="h-4 w-4 text-medical-teal" />
        ...
        <div className="flex gap-1 mb-8 bg-muted/50 p-1 rounded-xl w-fit overflow-x-auto">
          {navItems.map((item) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all whitespace-nowrap ${activeTab === item.id ? "bg-card text-primary shadow-card" : "text-muted-foreground hover:text-foreground"
                }`}
            >
              {item.label}
            </button>
          ...
          <div className="space-y-4">
            ...
            {showAddDoctor && (
              <div className="rounded-2xl bg-card border border-medical-teal/30 shadow-card p-6 animate-fade-in-up">
                ...
                <AddDoctorForm onSuccess={() => setShowAddDoctor(false)} />
              </div>
            )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    {["Doctor", "Specialty", "Patients", "TimeSlot", "Joined", "Status", "Actions"].map((h) => (
                      ...
                      <tr key={doc.id} className="hover:bg-muted/30 transition-colors">
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-semibold text-foreground">{doc.fullname}</p>
                          </div>
                        </td>
                        <td className="px-4 py-4 text-sm text-muted-foreground">{doc.specialty}</td>
                        <td className="px-4 py-4 text-sm text-muted-foreground">{doc.email}</td>
                        <td className="px-4 py-4 text-sm text-foreground font-medium">{doc.slotDuration} min</td>
                        <td className="px-4 py-4">
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <button className="p-1.5 rounded-lg text-muted-foreground hover:text-medical-red hover:bg-medical-red-light transition-colors" onClick={() => handleDeleteDoctor(doc.id, doc.fullname)}>
